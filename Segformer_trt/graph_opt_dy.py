...
def layerNormGenV2(model):
    """
    onnx合并LayerNorm，LayerNorm有传参
    """
    ...
    for i in range(len(nodes)-1):
        if (nodes[i].op_type=='ReduceMean' and \
            ...
            nodes[i+8].op_type=='Mul' and \
            nodes[i+8].op_type=='Add'):
            ...
            new_node = onnx.helper.make_node(
                'LayerNorm',
                name = 'LayerNorm_'+str(layernorm_count),
                inputs = [nodes[i].input[0],nodes[i+9].input[1],nodes[i+10].input[1]],
                outputs = [nodes[i+10].output[0]]
            )
            layernorm_count+=1
            nodes.remove(nodes[i])
            nodes.insert(i,new_node)
...
if __name__=='__main__':
    onnx_path = 'mit_b2_dy.onnx'
    save_onnx_path = 'mit_b2_dy_opt.onnx'
    model = onnx.load(onnx_path)
    model = updateNodeDy(model)
    model = layerNormGen(model)
    onnx.save(model,save_onnx_path)
    #model=layerNormGenV2(model)
    #onnx.save(model,save_onnx_path)
